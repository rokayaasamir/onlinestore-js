...
blackScreen.addEventListener("click", (eo) => {
  ...
  if (eo.target.classList.contains("btn-secondary")) {
    eo.target.parentElement.remove()
      ...
      const nameOfDeletProduct=eo.target.parentElement.getElementsByClassName("product-name")[0].innerText
    const allCardsiInGallary=document.querySelectorAll(".card")
    allCardsiInGallary.forEach(item =>{
      const nameOfProductInGalary=item.getElementsByClassName("card-title")[0].innerText

      if (nameOfDeletProduct==nameOfProductInGalary) {
...
popUp.innerHTML = "&#128525; لقد تم الشراء بنجاح "
setTimeout(() => {
